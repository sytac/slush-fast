...
gulp.task('dev-protractor-server', function(done) {
  ...
  protractorServer = server.listen(settings.servers.protractor.port,
    settings.servers.protractor.host,
    function() {
      console.log('Protractor running on ' + settings.servers.protractor.host +
        ':' + settings.servers.protractor.port);
      done();
    });
...
gulp.task('karma', function(done) {
    ...
    var files = bowerFiles
      .concat(['./' + generator.srcDir +
        '/**/!(*bootstrap|config|run|scenario).js',
...
gulp.task('dev-partials', function() {
  var streams = globule.find('./' + generator.srcDir + '/*')
    .map(function(file) {
      ...
      return gulp.src('./' + generator.srcDir + '/' + moduleDirName +
          '/*.html')
        .pipe(minifyHtml({
          empty: true,
          quotes: true,
...
gulp.task('watch-bootstrap', function() {
  watch(globs.bootstrap.src, function(files, done) {
    seq('dev-bootstrap', 'dev-js-template',
      'dev-js-config-run-template',
...
gulp.task('watch-partials', function() {
  watch([globs.templates.app.src, './' + generator.srcDir +
    '/.freak/partials'
  ], function(
    files,
    done) {
...
gulp.task('dev-server', function(done) {
  ...
  server.listen(settings.servers.dev.port, settings.servers.dev.host,
    function() {
      console.log('dev running on ' + settings.servers.dev.host + ':' +
        settings.servers.dev.port);
      done();
    });
...
gulp.task('dev-coverage-server', function(done) {
  ...
  coverageServer = server.listen(settings.servers.coverage.port, settings.servers
    .coverage.host,
    function() {
      console.log('Coverage running on ' + settings.servers.coverage.host +
        ':' + settings.servers.coverage.port);
      done();
    });
...
gulp.task('dev-jasmine-server', function(done) {
  ...
  jasmineServer = server.listen(settings.servers.jasmine.port, settings.servers
    .jasmine.host,
    function() {
      console.log('Jasmine running on ' + settings.servers.jasmine.host +
        ':' + settings.servers.jasmine.port);
      done();
    });
...
gulp.task('dev-coverage-browsersync', function() {
  browserSync({
    ...
    proxy: settings.servers.coverage.host + ':' + settings.servers.coverage
      .port,
    reloadDelay: 2000,
    open: false
  });
    ...
    var streams = globule.find(paths.js.dist + '/**/*.module.js')
      .map(function(file) {
        ...
        var jsStream = gulp.src(srcDir + '/*.js', './target/dist/app/' +
          ...
          .pipe(sourcemaps.write())
          .pipe(gulp.dest('./' + generator.distDir +
            '/modules/minified-sourcemapped'));

        var cssStream = gulp.src('./' + generator.srcDir + '/**/' +
            moduleName + '/*.scss')
          .pipe(sourcemaps.init())
          .pipe(sass())
          .pipe(rename(function(path) {
            path.dirname = '.';
            path.basename = generator.prefix + '.' + moduleName + '.' +
              path.basename;
          }))
          .pipe(gulp.dest('./' + generator.distDir + '/modules/packaged'))
          .pipe(cssmin())
...
function sanitizeGeneratorJson(generator) {
  ...
  if (typeof generator.distDir === 'undefined') {
    ...
    console.log(
      'Property distDir missing in generator.json, adding default with value \'dist\''
    );
